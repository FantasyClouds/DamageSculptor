...
public class NumericalMappingLoader {
    ...
    public static void reload(ResourceManager resourceManager) {
        ...
        NumericalSculptor.LOGGER.info("Found {} damage mapping file(s).", resources.size());

        for (ResourceLocation res : resources) {
            try {
                Optional<Resource> optResource = resourceManager.getResource(res);
                if (optResource.isEmpty()) {
                    NumericalSculptor.LOGGER.error("Resource not present: {}", res);
                    continue;
                }
                ...
                if (newConfig == null) {
                    NumericalSculptor.LOGGER.error("Failed to parse (null): {}", res);
                    continue;
                }

                if (!newConfig.isOverallValid()) {
                    NumericalSculptor.LOGGER.error("Invalid config, skipping. entity='{}', hasHealth={}, attemptsDamage={}. File: {}",
                ...
                if (existing == null) {
                    ...
                    NumericalSculptor.LOGGER.debug("Registered config for entity '{}' from {}", key, res);
                } else {
                    ...
                    if (newConfig.hasHealthConfig() && !existing.hasHealthConfig()) {
                        ...
                        NumericalSculptor.LOGGER.warn("Health config for entity '{}' already present, ignoring from {}", key, res);
                    }
                    ...
                    if (newConfig.hasValidDamageConfig() && !existing.hasValidDamageConfig()) {
                        ...
                        NumericalSculptor.LOGGER.warn("Damage config for entity '{}' already present, ignoring from {}", key, res);
                    }

                    if (merged) {
                        NumericalSculptor.LOGGER.info("Merged missing fields into entity '{}' from {}", key, res);
                    } else {
                        NumericalSculptor.LOGGER.debug("No new fields to merge for entity '{}' from {}", key, res);
                    }
                }

            } catch (IOException e) {
                NumericalSculptor.LOGGER.error("I/O error reading: {}", res, e);
            } catch (Exception e) {
                NumericalSculptor.LOGGER.error("Unexpected error: {}", res, e);
            }
        }

        NumericalSculptor.LOGGER.info("Damage mapping reloaded, {} entities configured.", MAPPINGS.size());
    }
